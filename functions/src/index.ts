...
// 1) 앱에서 호출하는 서버측 계정/데이터 삭제 (권장 진입점)
...
// 2) 계정 삭제 트리거(백업용). 앱/웹에서 계정을 직접 삭제해도 데이터가 지워지도록.
...
// 3) 웹 엔드포인트(플레이 콘솔 제출용 URL): GET=안내 페이지, POST=ID 토큰 인증 후 삭제
export const accountDeletion = functions.https.onRequest(async (req, res) => {
  // CORS
  ...
  if (req.method === 'GET') {
    ...
    res.status(200).send(`
      ...
      <body style="font-family:sans-serif; max-width:720px; margin:40px auto; line-height:1.6">
        ...
        <ol>
          <li>앱에서: 프로필 → 탈퇴 버튼으로 계정과 모든 데이터를 즉시 삭제할 수 있습니다.</li>
          <li>앱 접근이 어려울 경우:
            <ul>
              <li>아래 API에 Firebase ID 토큰으로 POST 요청을 보내 삭제를 진행할 수 있습니다.</li>
              <li>또는 support@yourdomain.com 으로 “계정 삭제 요청” 메일을 보내주세요.</li>
            </ul>
          </li>
        </ol>
